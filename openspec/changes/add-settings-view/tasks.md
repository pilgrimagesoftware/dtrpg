...
- [ ] 1.1 Create `dtrpg-ui/src/controllers/settings.rs` with `SettingsController` struct owning: active tab index, file-opener override list, and open/closed state
- [ ] 1.2 Add `SettingsTab` enum with variants `Account`, `Storage`, `FileOpeners`
- [ ] 1.3 Persist the last-active tab to the app config file on change; restore it on `SettingsController::new()`
- [ ] 1.4 Create `dtrpg-ui/src/ui/views/settings_view.rs` with an overlay panel that renders a tab strip and the active section content
...
- [ ] 1.6 Add `SettingsController` as a second `gpui::Entity` in `LibraryRootView`; conditionally render the settings panel overlay when `SettingsController::is_open()` is true
- [ ] 1.7 Add a settings/gear button to the toolbar that toggles `SettingsController::is_open`

## 2. Rust App — Account Section

- [ ] 2.1 Create `settings_account_view.rs` rendering account identity (from credential store or a stub "Signed in" label if account metadata is not yet available)
- [ ] 2.2 Implement "Log Out" button with a confirmation dialog; on confirm, call the credential store `delete()` and reset app session state
- [ ] 2.3 Implement "Reset API Key" button: clear the API key credential from the store and present the API key entry flow
- [ ] 2.4 Render an unauthenticated state ("Not signed in" + authenticate prompt) when no valid session is present
- [ ] 2.5 Wire account section to `SettingsController::Account` tab

## 3. Rust App — Storage Section

- [ ] 3.1 Create `settings_storage_view.rs` rendering the current `StorageConfig::root_path()` as a read-only path label
...
- [ ] 3.4 Wire storage section to `SettingsController::Storage` tab

## 4. Rust App — FileOpenerConfig Persistence

- [ ] 4.1 Define `FileOpenerEntry { extension: String, app_path: PathBuf }` and `FileOpenerConfig { entries: Vec<FileOpenerEntry> }` in `dtrpg-core` or `dtrpg-ui`
- [ ] 4.2 Implement `FileOpenerConfig::load()` and `save()` reading/writing the `[[file_openers]]` TOML array in the app config file
- [ ] 4.3 Implement `FileOpenerConfig::find_override(extension: &str) -> Option<&Path>` with case-insensitive, leading-dot-tolerant extension lookup
- [ ] 4.4 Implement `FileOpenerConfig::add(entry)` with duplicate-extension detection (replace after confirmation or reject)
- [ ] 4.5 Implement `FileOpenerConfig::remove(extension: &str)` and `update_app_path(extension: &str, new_path: PathBuf)`
- [ ] 4.6 Implement `FileOpenerConfig::validate_all() -> Vec<&FileOpenerEntry>` returning entries whose `app_path` does not exist on disk
- [ ] 4.7 Write unit tests for `find_override` (exact match, case-insensitive, no match, dot-prefixed input), `add` (new, duplicate), and `validate_all`

## 5. Rust App — File Openers Section UI

- [ ] 5.1 Create `settings_file_openers_view.rs` rendering the override list; show an empty-state message when no overrides are configured
- [ ] 5.2 Render each override as a row: extension badge, application name (derived from path), edit button, remove button
- [ ] 5.3 Flag invalid (stale) override rows with a warning indicator; call `FileOpenerConfig::validate_all()` when the section is first rendered
- [ ] 5.4 Implement "Add" control: present an inline row or dialog for extension input + application picker via `rfd::FileDialog` (filter to `.app` on macOS, executable on Windows/Linux)
- [ ] 5.5 Normalize the entered extension to lowercase with no leading dot before saving
- [ ] 5.6 Implement edit: re-open the application picker pre-filled with the current path; update the entry on confirmation
- [ ] 5.7 Implement remove: delete the entry from `FileOpenerConfig` and refresh the list
- [ ] 5.8 Wire File Openers section to `SettingsController::FileOpeners` tab
