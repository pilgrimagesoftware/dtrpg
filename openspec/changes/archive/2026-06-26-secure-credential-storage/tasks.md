...
- [x] 1.1 Add `keyring` crate dependency to `dtrpg-app/rust/Cargo.toml` (enable appropriate platform features)
- [x] 1.2 Define `Credential` value type with `service`, `account`, and `secret` fields in `dtrpg-app/rust`
- [x] 1.3 Define `CredentialError` error enum using `thiserror` covering store/load/delete failure cases
- [x] 1.4 Define `CredentialStore` trait with `store`, `load`, and `delete` methods returning `Result<_, CredentialError>`
- [x] 1.5 Implement `KeyringCredentialStore` struct that wraps `keyring::Entry` and satisfies `CredentialStore`
- [x] 1.6 Write unit tests for `KeyringCredentialStore` (mock or integration-mode against real keyring in CI)

## 2. Rust App — Namespace and Key Schema

- [x] 2.1 Define constants for service name (`com.pilgrimagesoftware.dtrpg`) and account key names (`api-key`, `access-token`, `refresh-token`) in a `credentials::keys` module
- [x] 2.2 Verify all `CredentialStore` call sites use the constants, not inline strings
...
- [x] 6.1 Confirm the Rust app builds successfully on macOS (`aarch64-apple-darwin`) with `keyring` linking `Security.framework`
...
- [x] 7.1 Audit `dtrpg-sdk/rust` for any code that writes auth tokens or credentials to disk
- [x] 7.2 If found, refactor to remove persistence from the SDK and document that callers must provide storage
- [x] 7.3 Update SDK documentation to clarify that credential storage is the caller's responsibility

## 8. Cleanup and Documentation

- [x] 8.1 Confirm no plaintext credential paths remain in config file parsing code (grep for field names like `api_key`, `password`, `token` in config loading)
- [x] 8.2 Update the app's README / installation docs with the Linux `libsecret` requirement
- [x] 8.3 Document the `com.pilgrimagesoftware.dtrpg` namespace convention in the relevant `CLAUDE.md` or architecture notes for future contributors
