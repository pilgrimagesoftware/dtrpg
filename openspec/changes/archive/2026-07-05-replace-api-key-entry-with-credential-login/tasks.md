...
- [x] 5.1 Run `cargo test --workspace` in `dtrpg-sdk/rust` and `dtrpg-app/rust`
- [x] 5.2 Manually verify sign-in with valid and invalid credentials against the real
      DriveThruRPG website endpoints
- [x] 5.3 Verify a pre-existing legacy (key-only) keychain entry still allows silent
