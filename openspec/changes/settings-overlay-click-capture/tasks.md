...
- [ ] 1.1 In `render_settings_panel` in `dtrpg-ui/src/ui/views/settings_view.rs`, locate the outermost backdrop `div()` (the one with `.absolute().inset_0().bg(backdrop)`)
- [ ] 1.2 Add `.id("settings-backdrop")` to that `div` so gpui registers it in the hitbox tree
- [ ] 1.3 Add `.occlude()` after `.id(...)` to set `HitboxBehavior::BlockMouse`, which prevents all pointer events and hover states from reaching elements behind the backdrop
